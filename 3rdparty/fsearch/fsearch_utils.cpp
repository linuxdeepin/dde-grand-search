...
bool convert_chinese_2_pinyin(const char *in_str, char *out_first_py, char *out_full_py)
{
    ...
    if (ret) {
        int full_py_len = full_py.size();
        int first_py_len = first_py.size();

        memcpy(out_full_py, full_py.toLocal8Bit().data(), full_py_len > FILENAME_MAX ? FILENAME_MAX : full_py_len);
        memcpy(out_first_py, first_py.toLocal8Bit().data(), first_py_len > FILENAME_MAX ? FILENAME_MAX : first_py_len);
    }
