...
gpointer
load_database (Database **db, const char *search_path)
{
    if (!*db) {
        // create new database
        timer_start ();
        *db = db_new ();
        ...
        if (!db_location_load (*db, search_path)) {
            if (db_location_add (*db, search_path, build_location_callback)) {
                loaded = true;
                build_new = true;
            }
        } else {
        ...
        timer_start ();
        db_clear (*db);
        db_location_add (*db, search_path, build_location_callback);
        db_build_initial_entries_list (*db);
        timer_stop ();
    }
    return NULL;
