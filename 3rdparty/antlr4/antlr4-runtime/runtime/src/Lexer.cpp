...
void Lexer::emit1(std::unique_ptr<Token> newToken) {
  token = std::move(newToken);
}

Token* Lexer::emit1() {
  emit1(_factory->create({ this, _input }, type, _text, channel,
...
Token* Lexer::emitEOF() {
  ...
  size_t line = getLine();
  emit1(_factory->create({ this, _input }, EOF, "", Token::DEFAULT_CHANNEL, _input->index(), _input->index() - 1, line, cpos));
  return token.get();
}
